...
class Mobileapi extends CI_Controller {
	...
	public function logIn()
	{
		...
		if($this->MobileapiModel->logIn($email, $password)){
	...
	public function insertCustomerOrder()
	{
		...
		if($email == ""){
			...
			$insertedCustomerId = $this->MobileapiModel->insertCustomer($name,$email,$phone,$shipping_address);
			if($insertedCustomerId){
				...
				$id_order = $this->MobileapiModel->insertOrder($customer_id, $amount, $quantity, $extra_message, $device_id, $shipping_cost);
				if($id_order){
...
//
	...
	public function signUp()
	{
		...
		if($pos !== false){
			...
			$data = [];
			$data['name'] = $submitted_data['name_et_str'];
			$data['phone'] = $submitted_data['phone_et_str'];
			$data['email'] = $this->input->post('email_et_str', TRUE);
			$data['password'] = $submitted_data['pass_et_str'];
			$data['address'] = $submitted_data['address_et_str'];
	...
	public function getItemImages()
	{
		...
		echo json_encode($this->MobileapiModel->getItemImages($id_item));
	}
}
